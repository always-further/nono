...
---
*Last updated: 2026-05-09 after Phase 31 close (Broker-Process Architecture, SHELL-01 → ✔ validated v2.3 Phase 31). Phase 31 productionized the broker-pattern PoC validated on the user's Windows test box on 2026-05-08 (commit 98d38ed9). The new `crates/nono-shell-broker/` workspace member is a Medium-IL intermediary spawned by `nono.exe` that lowers a duplicate of its own token to Low Mandatory Integrity Level (`SetTokenInformation(TokenIntegrityLevel, WinLowLabelSid)` against a `DuplicateTokenEx(SecurityAnonymous, TokenPrimary)` clone), then calls `CreateProcessAsUserW(low_il_token, ..., dwCreationFlags=EXTENDED_STARTUPINFO_PRESENT)` to launch the actual shell child. The Low-IL grandchild inherits the broker's already-attached console, so KernelBase's `ConClntInitialize` short-circuits the CSRSS ALPC connect (RESEARCH Assumption A1, validated end-to-end on the production binary chain). `WindowsTokenArm::BrokerLaunch` cascade arm dispatch is active in `crates/nono-cli/src/exec_strategy_windows/launch.rs`; sibling broker resolution via `current_exe()` (D-07) with `NonoError::BrokerNotFound` fail-closed if `nono-shell-broker.exe` is missing. Job Object containment cascades from `nono.exe` → broker → Low-IL shell child because `JOB_OBJECT_LIMIT_*BREAKAWAY*` flags are unset (D-04 acceptance closed via lifted `IsProcessInJob` runtime test in `broker_dispatch_tests` 2/2 PASS). Phase 31 field validation: Acceptance #1 (no 0xC0000142, no silent exit), #2 (claude TUI rendering), #3 (Set-Content write-deny outside grant set OS-denied), #4 (read of granted ~/.claude\claude.json works), #7 (corrected `Set-Content -Path -Value` harness — distinguishes OS-deny from PowerShell parse-error) all PASS on user's test box on 2026-05-09. Cookbook (`docs/cli/development/windows-poc-handoff.mdx`) updated with the Phase 31 security envelope: token shape, what's enforced at OS level (mandatory-label NO_WRITE_UP via MIC kernel pre-DACL check; Job Object containment), what's defense-in-depth (Claude Code `PreToolUse` hook is UX + audit, NOT primary boundary), what's NOT enforced (read-deny across IL boundary; per-session WFP differentiation waived on broker path — falls back to AppID). Authenticode chain-walker (Phase 28) records consistent signer for `nono.exe` AND broker (same Authenticode key, consistent identity in audit trail). Debug session `.planning/debug/resolved/nono-shell-status-dll-init-failed.md` Postscript 2 marks the line of bug → PoC → Phase 31 closure. Cumulative v2.3 progress: 11/17 requirements closed + SHELL-01 validated (REQ-AIPC-NIX-01 / REQ-AUDC-01..03 / REQ-WRU-01..02 / REQ-PKGS-02..03 / REQ-NTH-01..02 + SHELL-01). Phase 30 had no formal REQ-IDs (decision-coverage gate via D-01..D-10; all 10 D's addressed per 30-VERIFICATION.md). Phase 31 also tracked via decision-coverage (D-01..D-16, all addressed). Remaining v2.3: REQ-RESL-NIX-01..03 (Phase 25 Plan 25-01, queued for Linux/macOS host), REQ-PKGS-01 + REQ-PKGS-04 (Phase 26 Plan 26-02), REQ-AAH-01 (Phase 27 — Phase 27.2 closes via Phase 27.1 NONO_TEST_HOME seam). Phase 31 is the v2.3 milestone's last Windows-host-required phase; v2.3 closes via `/gsd-complete-milestone v2.3` once Phases 25-01 / 26-02 / 27.2 land on Linux/macOS host.*
