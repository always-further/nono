...
Load credentials from the system keystore (macOS Keychain / Linux Secret Service), 1Password, or Apple Passwords (macOS) and inject them as environment variables. The sandboxed process can read these credentials directly.
...
# Load from Apple Passwords on macOS (apple-password:// URI with =VAR_NAME suffix)
nono run --allow-cwd --env-credential 'apple-password://github.com/alice@example.com=GITHUB_PASSWORD' -- my-agent
...
- URI manager entries (`op://...`, `apple-password://...`) require explicit `=VAR_NAME`
