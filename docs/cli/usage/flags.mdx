...
## `nono profile` Options

### `--draft`

Routes `nono profile init` and `nono profile validate` to the `profile-drafts/`
directory (sibling of `profiles/`) instead of the canonical profiles dir. Drafts
are a review-driven workflow: create with `init --draft <name>`, edit, then
`nono profile promote <name>` to apply.
...
(Requires `--draft`.) Re-reads the current canonical profile, recomputes the
SHA-256 base-hash, and rewrites the draft's sidecar `<name>.base` file. Preserves
the draft JSON content — use to reconcile a stale base-hash after the canonical
profile changes underneath you.
...
(On `nono profile promote` only.) Skip the interactive `[y/N]` review prompt;
promote unconditionally on hash-match. Does NOT bypass shadow-detection or
base-hash verification — those checks always fail-closed.
...
The override path must also be explicitly granted via `--allow`, `--read`, `--write`, or their file equivalents. `--bypass-protection` only removes the deny rule — it does not implicitly grant access.
...
<Note>
  **Legacy alias (D-36-B3):** The original flag name `--override-deny` (pre-v0.47.0) is retained as a `visible_alias` indefinitely per D-36-B3 — both names work with no behavior difference. Canonical name since v0.47.0 (upstream f0abd413, Plan 36-01c): `--bypass-protection`. Deprecated: `--override-deny`.
</Note>
...
These flags control which host environment variables the sandboxed child inherits. They run **before** the sandbox is launched, so the filter applies to the very first `exec` of the sandboxed command.

#### `--env-allow`

Allow-list pattern for environment variables inherited by the sandboxed child. Accepts an exact name or a trailing-prefix glob (e.g. `AWS_*`). Repeatable. Malformed patterns fail closed at parse time. When any `--env-allow` is set, only variables matching at least one pattern are inherited; everything else is removed before `exec`.

```bash
# Only pass through AWS_* variables and a specific PATH
nono run --env-allow 'AWS_*' --env-allow PATH --allow-cwd -- my-agent

# Combine with deny to forbid known-sensitive subsets
nono run --env-allow '*' --env-deny 'AWS_*' --allow-cwd -- my-agent
```

#### `--env-deny`

Deny-list pattern for environment variables inherited by the sandboxed child. Accepts an exact name or a trailing-prefix glob (e.g. `AWS_*`). Repeatable. Malformed patterns fail closed at parse time. When both `--env-allow` and `--env-deny` match, deny takes precedence — this prevents an overly-broad allow from accidentally admitting something listed in deny.

```bash
# Strip all AWS_* variables, pass everything else
nono run --env-deny 'AWS_*' --allow-cwd -- my-agent

# Strip credentials and tokens
nono run --env-deny 'AWS_*' --env-deny 'GITHUB_TOKEN' --allow-cwd -- my-agent
```
...
Disable cargo-install-style auto-pull when `--profile` references a registry pack
that is not yet installed locally. With this flag set, nono falls back to the
legacy `profile not found` error instead of fetching the pack from the registry.

```bash
nono run --no-auto-pull --profile namespace/foo -- my-agent
```

Useful for audit/airgap workflows and CI environments where implicit network
access during profile resolution is undesirable. The flag is accepted on
`nono run` and `nono wrap` only; `nono pull` is an explicit-install command and
does not get the flag.

| Environment Variable | `NONO_NO_AUTO_PULL` |
|---------------------|---------------------|
| Example | `NONO_NO_AUTO_PULL=1` |
...
Override the rollback snapshot destination directory. By default, snapshots are stored under the platform's nono rollback directory. The destination must be within a path already granted write access via `--allow` (or by an active profile); nono fails with a clear error if not. Useful for Docker volume mounts or shared storage paths.

```bash
# Store snapshots in an explicitly granted shared volume
nono run --rollback --allow /mnt/snapshots --rollback-dest /mnt/snapshots --allow-cwd -- my-agent
```

<Note>
  `--rollback-dest` requires `--rollback`. The destination directory inherits sandbox permission rules: writes to it from inside the sandbox MUST be covered by an existing capability grant.
</Note>
...
These flags cap CPU, memory, and process resources for the sandboxed agent tree. On Windows they are kernel-enforced through Job Objects (REQUIREMENTS.md § RESL-01..RESL-03). On Linux/macOS they are accepted with a warning pending cross-platform follow-up.

#### `--cpu-percent`

Cap the sandboxed agent tree's CPU usage as a percentage of one logical CPU (1..=99). Kernel-enforced on Windows via `CpuRateControlInformation`. On Linux/macOS the value is accepted with a warning pending cross-platform follow-up.

```bash
# Limit a build to ~50% of one core
nono run --cpu-percent 50 --allow-cwd -- cargo test
```

#### `--memory`

Cap the sandboxed agent tree's total memory. Accepts size suffixes (`512M`, `1G`, `256K`, `2T`) or raw bytes. Kernel-enforced job-wide on Windows via `JobMemoryLimit`. On Linux/macOS the value is accepted with a warning pending cross-platform follow-up. See REQUIREMENTS.md § RESL-02.

```bash
# Cap memory at 2 GiB
nono run --memory 2G --allow-cwd -- cargo build

# Raw byte value (1 MiB)
nono run --memory 1048576 --allow-cwd -- my-agent
```

#### `--max-processes`

Cap the number of active processes in the sandboxed agent tree. Range: 1..=65535. Kernel-enforced on Windows via `ActiveProcessLimit`. On Linux/macOS the value is accepted with a warning pending cross-platform follow-up.

```bash
# Hard-cap a noisy build at 32 processes
nono run --max-processes 32 --allow-cwd -- make -j32
```
...
| `--no-auto-pull` | `NONO_NO_AUTO_PULL` | `NONO_NO_AUTO_PULL=1` |
