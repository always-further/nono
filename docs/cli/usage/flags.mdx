...
Allow outbound localhost connections to specific destination ports through the parent relay. Repeatable.

```bash
# Allow sandbox to connect to localhost:11435 only
nono run --allow-cwd --net-block \
  --experimental-localhost-relay --allow-port 11435 \
  -- curl http://localhost:11435/

# Allow multiple localhost destination ports
nono run --allow-cwd --net-block \
  --experimental-localhost-relay --allow-port 11435 --allow-port 11439 \
  -- my-agent
```

<Note>
`--allow-port` is currently experimental and requires `--experimental-localhost-relay`.
</Note>

#### `--experimental-localhost-relay`

Enable experimental localhost relay mode for `nono run`. Localhost traffic is routed through the parent proxy, and only ports granted with `--allow-port` are reachable.

```bash
nono run --allow-cwd --net-block \
  --experimental-localhost-relay --allow-port 11439 \
...
- Requires `--net-block`.
- Requires at least one `--allow-port`.
- Supported only with `nono run` (not `nono shell`).
- Cannot be combined with `--network-profile`, `--proxy-allow`, `--proxy-credential`, `--external-proxy`, or `--proxy-port`.
</Note>
