...
```bash
# CA valid for 7 days (useful for long-running sessions)
nono run --credential github --proxy-ca-validity 7 -- gh api /user
```
...
- **Linux only**, enforced via cgroup v2, and requires a systemd `Delegate=yes` user session. On other platforms the request is refused rather than silently ignored.
- Enforced on `nono run` and `nono shell` (supervised). `nono wrap` execs directly and does not accept `--memory`.
- Granting the sandbox **write** access overlapping `/sys/fs/cgroup` (e.g. `--allow /sys`) is refused while `--memory` is active, since it would let the process rewrite its own cap. Use `--read /sys` instead if a workload needs it.

See [Resource Limits](/cli/features/resource-limits) for the full details.
...
Without this flag, the proxy uses HTTP/1.1 with keep-alive connection pooling. This is the conservative default since some upstream servers may not handle HTTP/2 correctly.

See [Networking — HTTP/2 Support](/cli/features/networking#http2-support---allow-http2) for details on connection pooling and profile configuration.
...
Set a startup timeout in seconds for the sandboxed process. If the process does not appear interactive within the given time, nono prints a warning and terminates it. Entering alt-screen or writing visible non-whitespace output both count as interactive. Set to `0` to disable entirely.
...
On macOS, to hide specific expected Seatbelt violations (such as
`user-preference-read` from npm) without silencing all diagnostics, add the
operation name to `diagnostics.suppress_system_services` in your profile
instead. See [Profiles and Groups](/cli/features/profiles-groups#suppressing-expected-macos-system-service-diagnostics).
...
| Value | Behaviour |
|-------|-----------|
| `auto` | Landlock LSM with automatic seccomp fallback when the kernel ABI lacks network support (< v4). **Default.** Preserves existing behaviour. |
| `landlock` | Landlock only. Returns an error at startup if the running kernel cannot satisfy network restrictions via Landlock alone — no silent seccomp fallback. |
| `external` | No-op. nono installs no Landlock or seccomp rules. Use when enforcement is managed at the infrastructure level (iptables, cgroups, systemd units, container runtime policy). |

```bash
# Require Landlock-only enforcement; fail if the kernel can't satisfy it
nono run --sandbox-policy landlock --allow-cwd -- my-agent

# Declare that enforcement is external; nono skips sandbox installation
nono run --sandbox-policy external --allow-cwd -- my-agent
```

The policy can also be set in a profile under `linux.sandbox_policy`. The CLI flag overrides the profile value. On macOS and other non-Linux platforms this flag is not available.
...
<Warning>
  `--sandbox-policy external` provides no kernel-level enforcement on its own. Only use it when you have independently verified that the surrounding infrastructure enforces the required policy.
</Warning>
