...
<Note>
  `nono wrap` does not support proxy flags (`--network-profile`, `--allow-proxy`, `--proxy-credential`, `--external-proxy`). The network proxy requires a parent process. Use `nono run` instead.
</Note>
...
<Note>
  `--allow-net` disables proxy mode for that run, so it also disables proxy-based credential injection. It conflicts with `--block-net`, `--network-profile`, `--allow-proxy`, `--proxy-credential`, `--external-proxy`, and `--proxy-port`.
</Note>
...
Available profiles: `claude-code`, `developer`, `minimal`, `opencode`, `enterprise`. See [Network Proxy](/cli/features/network-proxy) for details.

<Note>
  `--network-profile` and `--allow-proxy` activate proxy mode, which forces supervised execution. The proxy runs in the unsandboxed parent process.
</Note>
...
nono run --allow-cwd --allow-proxy api.openai.com --allow-proxy api.anthropic.com -- my-agent

# Combine with a network profile for additional hosts
nono run --allow-cwd --network-profile minimal --allow-proxy custom-api.example.com -- my-agent
```
...
<Note>
`--allow-bind` only has effect in proxy mode (when `--network-profile` or `--allow-proxy` is active). Without proxy mode, network operations use the default OS-level allow/deny and bind is not restricted.
</Note>
...
nono run --block-net --allow-port 3000 --allow ./mcp-server -- node server.js
nono run --block-net --allow-port 3000 --allow ./client -- claude

# Multiple IPC ports
nono run --block-net --allow-port 3000 --allow-port 5000 --allow-cwd -- my-agent
...
| `--allow-net` | `NONO_ALLOW_NET` | `NONO_ALLOW_NET=true` |
