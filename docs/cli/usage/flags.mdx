...
Enable experimental proxy-mediated localhost relay for `nono run`. In this mode, localhost HTTP and CONNECT traffic is forced through the parent proxy, and `--allow-port` becomes the destination-port allowlist enforced by that relay.
