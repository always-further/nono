...
Explicit repository root for `$REPO_ROOT` expansion in profile paths. When omitted, nono auto-detects the root via `git rev-parse --show-toplevel` from the current directory. Set `NONO_REPO_ROOT` as an environment variable alternative. Paths containing `$REPO_ROOT` are silently skipped when the root cannot be determined.

```bash
nono run --profile always-further/claude --repo-root /path/to/repo -- claude
```
