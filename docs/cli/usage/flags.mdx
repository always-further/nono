...
Exclude a directory pattern from rollback snapshots. Repeatable. Patterns without `/` match exact path components; patterns with `/` match as substrings. Does NOT affect sandbox permissions — excluded directories are still sandboxed.
...
Exclude files matching a glob pattern from rollback snapshots. Repeatable. Matched against filenames only. Does NOT affect sandbox permissions.

```bash
# Exclude log files from rollback
nono run --rollback --rollback-exclude-glob "*.log" -- my-agent
```
