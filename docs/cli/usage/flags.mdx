...
Compose the selected `--profile` with an additional base profile for this comparison. Repeatable; bases are applied left-to-right before any `extends` already declared by the selected profile. Requires `--profile`.

```bash
nono learn --profile my-agent --extends linux-host-compat -- my-agent
```
