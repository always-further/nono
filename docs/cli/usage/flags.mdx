...
Set up nono on this system. Verifies installation, tests sandbox support, optionally generates example profiles, and can export the embedded sandbox policy.
...
`nono shell` supports the same permission, profile, policy (`--sandbox-policy`), secrets, and dry-run flags as `nono run`, plus:
...
Load sandbox policy from a JSON file instead of the embedded policy. When set, this file takes precedence for groups, profiles, deny paths, and blocked commands for that invocation.

```bash
nono run --sandbox-policy ./policy.json --profile claude-code -- claude
nono shell --sandbox-policy ./policy.json --profile claude-code
```
...
Export embedded `policy.json` to disk. If `FILE` is omitted, writes `./policy.json`.
