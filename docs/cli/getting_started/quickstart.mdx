...
For granular control, use `--network-profile` for host-level filtering, `--net-allow` to override proxy filtering for one session, or `--allow-port` for localhost IPC between sandboxes:

```bash
# Host-level filtering via proxy
nono run --allow . --network-profile claude-code -- my-agent

# Keep a built-in profile but temporarily disable its proxy filtering
nono run --profile claude-code --net-allow -- claude

# Localhost IPC (e.g., MCP server on port 3000)
nono run --net-block --allow-port 3000 --allow . -- my-agent
```
