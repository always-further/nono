...
| `nono setup` | System setup and verification - generate profiles, check shell integration, export policy.json |

## Custom Sandbox Policy

The sandbox policy is embedded in `nono` by default. To customize it without recompiling:

1. Export the embedded policy:
   ```bash
   nono setup --export-policy ./policy.json
   ```
2. Edit `policy.json` (groups, profiles, command blocklist, deny paths).
3. Use it for a specific invocation:
   ```bash
