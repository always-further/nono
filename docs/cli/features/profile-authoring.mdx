...
The `credential_capture` section maps logical names to commands. The command is resolved to an absolute path at profile-load time, and the result is cached for the specified TTL. See [Credential Injection — CLI Command Capture](/cli/features/credential-injection#cli-command-credential-capture-cmd) for full details.
