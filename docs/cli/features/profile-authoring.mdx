...
<Note>
  `bypass_protection` only removes the deny rule. You must also grant access via `filesystem` or `policy.add_allow_*` for the path to be accessible. Canonical key since v0.47.0 (upstream f0abd413). The legacy `override_deny` — Deprecated, accepted indefinitely via serde alias per D-36-B3.
</Note>
