...
| `override_deny` | Paths to exempt from deny groups (must also have a matching grant). **As of v0.47.0 (#594)**, the canonical key is `bypass_protection`; the legacy `override_deny` continues to deserialize for backwards compatibility, emitting a one-time stderr deprecation warning per process. The full canonical-schema migration (canonical sections `groups`, `commands`, `filesystem`) is tracked as a follow-up plan. Legacy keys will be removed in a future major release. |
