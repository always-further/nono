...
| Built-in profiles | Embedded `policy.json` (default) or `--sandbox-policy <FILE>` | Base - audited defaults (or explicit custom policy) |

CLI flags always override profile settings.

You can export the embedded policy and run with a customized policy file:
