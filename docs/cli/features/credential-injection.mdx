...
| `credential_key` | Yes | - | Keystore account name (alphanumeric and underscores only), `op://` URI for [1Password](#1password-integration), `apple-password://` URI for [Apple Passwords](#apple-passwords-integration-macos), `file://` URI for file-backed secrets, `env://` URI for host environment variables (e.g., `env://MY_TOKEN`), or `cmd://` URI for [CLI command capture](#cli-command-credential-capture-cmd) (e.g., `cmd://github`) |
...
## CLI Command Credential Capture (`cmd://`)

For credentials that live behind a CLI tool (`gh auth token`, `gcloud auth print-access-token`, `aws sts get-session-token`), nono can run the command on the host and inject the result — without the sandboxed process ever seeing the command or its output.

Use `cmd://<name>` as the `credential_key` and define the command in the `credential_capture` section:
...
1. The agent sends a request to `http://127.0.0.1:<port>/github/...`
2. The proxy validates the phantom token, then asks the supervisor to capture the credential
3. The supervisor runs `gh auth token` on the host (resolved to an absolute path at profile-load time)
4. The captured token is injected as `Authorization: token ghp_...` and forwarded to GitHub
5. The result is cached for 15 minutes (configurable via `ttl_secs`)

The sandboxed process never sees the real credential or the capture command.

### `credential_capture` Fields

| Field | Required | Default | Description |
|-------|----------|---------|-------------|
| `command` | Yes | — | Array of command + arguments. First element is resolved via PATH at profile load time. No shell interpolation. |
| `timeout_secs` | No | `5` | Maximum seconds to wait for the command (range: 1–300) |
| `ttl_secs` | No | `900` | How long to cache the result in seconds (range: 0–3600, 0 = no cache) |
| `cache_path_pattern` | No | — | Regex applied to the request path. First capture group becomes part of the cache key (see [Request-Aware Caching](#request-aware-caching)). |

### Request Context Environment Variables

When the credential capture command is executed, the following environment variables are set on the child process to provide request context:

| Variable | Description | Example |
|----------|-------------|---------|
| `NONO_REQUEST_HOST` | Upstream host (or `host:port`) the request is being forwarded to | `api.github.com` |
| `NONO_REQUEST_PATH` | Request path | `/repos/owner/name` |
| `NONO_REQUEST_METHOD` | HTTP method | `GET` |

These allow the credential command to dynamically produce different credentials based on what the agent is requesting — for example, returning a scoped token appropriate for the target API endpoint.

### Request-Aware Caching

By default, the captured credential is cached per route (credential name) within a session. If your command returns different credentials depending on the request path, use `cache_path_pattern` to partition the cache:

```json
{
  "credential_capture": {
    "scoped": {
      "command": ["my-token-script"],
      "cache_path_pattern": "^/([^/]+)",
      "ttl_secs": 300
    }
  }
...
The regex is applied to `NONO_REQUEST_PATH`. The **first capture group** becomes part of the cache key:

- `/repos/owner/a` and `/repos/owner/b` → cache key suffix `repos` (same cached credential)
- `/users/me` → cache key suffix `users` (separate cached credential)

If no pattern is configured, or the regex does not match, the cache key is just the credential name (one cached value per route per session). The regex must contain at least one capture group; invalid patterns are rejected at profile-load time.

### Security

- **Commands are allow-listed in the profile.** The sandboxed process names a logical credential (e.g., "github"), not a command. The supervisor maps name → command via the profile.
- **Absolute paths at load time.** Commands are resolved via PATH once at profile-load time and stored as absolute paths. No PATH lookup at capture time.
- **No shell interpretation.** Arguments are passed directly to `execve`-style invocation — no shell metacharacters, no variable expansion.
- **Stderr is never returned.** Command stderr is logged (scrubbed) for debugging but never sent to the proxy or child.
- **Credentials are zeroized.** All credential values use `Zeroizing<String>` and are cleared from memory after use or on session exit.

### Compared to Other Credential Sources

| Source | When Loaded | Where Resolved |
|--------|-------------|----------------|
| Keyring / `env://` / `file://` | Eagerly at proxy startup | Proxy process |
| `op://` / `apple-password://` | Eagerly at proxy startup | Proxy process (via CLI subprocess) |
| `cmd://` | **Lazily on first request** | Supervisor process (via capture channel) |

`cmd://` credentials are fetched just-in-time, reducing the window during which secrets live in memory. Subsequent requests use the cached value until TTL expires.
