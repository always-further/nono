...
| `env_var` | Conditional | - | Explicit environment variable name for the phantom token. Required when `credential_key` is `op://`, `bw://`, `apple-password://`, `file://`, or `cmd://`. Optional for `env://` (derived from the URI) |
...
|----------|-------------|
| `NONO_SESSION_ID` | Current nono session id. |
| `NONO_REQUEST_HOST` | Upstream request host. |
| `NONO_REQUEST_PATH` | Upstream request path. |
| `NONO_REQUEST_METHOD` | HTTP method. |
| `NONO_CACHE_SCOPE` | Derived cache scope. Defaults to host unless `cache_path_regex` matches. |
| `NONO_CAPTURE_CREDENTIAL` | Logical capture entry name. |
| `NONO_CAPTURE_ROUTE` | Proxy route id. |

The `command` array is resolved at capture time: bare `$VAR` tokens in any element are expanded from the **nono process environment** (i.e. the environment of the launcher, not the sandboxed child). Unset variables are left as-is.
...
The command receives JSON containing `session_id`, `credential_name`, `route_id`, `request_host`, `request_path`, `request_method`, and `cache_scope`.
