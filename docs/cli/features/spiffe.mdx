...
- **`spiffe` block** — for upstreams that accept SPIFFE credentials directly (mTLS or JWT bearer).
...
| `svid_hint` | X.509 | Select a specific SVID when the Workload API returns multiple |

`svid_hint` is useful when the SPIRE agent is configured to issue more than one SVID for the same
workload — for example, one for an internal service mesh and one for an external API. Set it to the
hint string assigned to the SVID you want nono to use; if absent, nono uses the first SVID returned.
...
- Fetching and injecting the right credential (JWT bearer, OAuth2 access token, or X.509 client cert)
...
5. For `client_assertion` flows: configure a federation rule or JWT verification policy on the
