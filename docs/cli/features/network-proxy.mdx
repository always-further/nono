...
`--allow-net` is a per-session override. It keeps the selected profile's filesystem and command policy, but disables proxy filtering and proxy-based credential injection for that run.
...
```bash
nono run --allow-cwd --allow-proxy api.openai.com -- curl https://api.openai.com/v1/models
```
