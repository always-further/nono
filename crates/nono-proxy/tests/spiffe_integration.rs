...
use nono_proxy::spiffe::{SpiffeJwtSource, SpiffeX509Source};
...
#[tokio::test]
async fn test_spiffe_x509_live_connect_and_available() {
    let socket = match live_socket() {
        Some(s) => s,
        None => return,
    };

    let src = SpiffeX509Source::connect("test-route", &socket, None, None, None)
        .await
        .expect("should connect to live SPIRE agent");

    assert!(
        src.is_available(),
        "X.509 source should be available after connect"
    );

    let spiffe_id = src.spiffe_id();
    let expected = live_expected_spiffe_id();
    assert_eq!(
        spiffe_id, expected,
        "X.509 SVID should carry the registered SPIFFE ID"
    );

    let _connector = src.tls_connector();
}
