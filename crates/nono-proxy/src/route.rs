...
impl std::fmt::Debug for LoadedRoute {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let managed_auth_type = self.managed_auth.as_ref().map(|a| match a.as_ref() {
            crate::auth::ManagedUpstreamAuth::SpiffeX509(_) => "spiffe_x509",
...
fn auth_mechanism_for_route(route: &RouteConfig) -> Option<NetworkAuditAuthMechanism> {
    if let Some(spiffe) = &route.spiffe {
        return Some(match spiffe {
            SpiffeAuthConfig::X509 { .. } => NetworkAuditAuthMechanism::SpiffeX509Mtls,
            SpiffeAuthConfig::Jwt { .. } => NetworkAuditAuthMechanism::SpiffeJwtBearer,
        });
    }
...
fn injection_mode_for_route(route: &RouteConfig) -> Option<NetworkAuditInjectionMode> {
    if let Some(spiffe) = &route.spiffe {
        return match spiffe {
            // X.509-SVID auth is transport-layer (mTLS), no header injection.
            SpiffeAuthConfig::X509 { .. } => None,
            SpiffeAuthConfig::Jwt { .. } => Some(NetworkAuditInjectionMode::SpiffeJwt),
        };
    }
...
impl RouteStore {
    ...
    pub async fn load(routes: &[RouteConfig]) -> Result<Self> {
        ...
        for route in routes {
            ...
            // SPIFFE is mutually exclusive with all other managed credential sources.
                    ...
                    || route.aws_auth.is_some())
            {
                return Err(ProxyError::Config(format!(
                    "route '{}': `spiffe` is mutually exclusive with `credential_key`, `oauth2`, and `aws_auth`",
                    normalized_prefix
                )));
            }
            ...
            let managed_auth: Option<Arc<crate::auth::ManagedUpstreamAuth>> = match &route.spiffe {
                Some(SpiffeAuthConfig::X509 {
                    workload_api_socket,
                    svid_hint,
                    expected_upstream_spiffe_id,
                }) => {
                    if expected_upstream_spiffe_id.is_none() {
                        warn!(
                            "route '{}': SPIFFE X.509 route has no expected_upstream_spiffe_id — \
                             any workload in the trust domain is accepted as upstream",
                            normalized_prefix
                        );
                    }
                    debug!(
                        "Connecting SPIFFE X.509 source for route '{}'",
                        normalized_prefix
                    );
                    let src = crate::spiffe::SpiffeX509Source::connect(
                        &normalized_prefix,
                        workload_api_socket,
                        svid_hint.as_deref(),
                        expected_upstream_spiffe_id.clone(),
                        route.tls_ca.clone(),
                    )
                    .await
                    .map_err(|e| {
                    ...
                    Some(Arc::new(crate::auth::ManagedUpstreamAuth::SpiffeX509(
                        Arc::new(src),
                    )))
                }
        ...
        // Reject configs where two X.509 SPIFFE routes share an upstream but
        // disagree on expected_upstream_spiffe_id. On h2, one TLS connection
        // is shared across all streams, so the first-match peer ID wins and
        // silently violates the second route's intent.
        let mut upstream_peer_id: std::collections::HashMap<String, (Option<String>, String)> =
            std::collections::HashMap::new();
        for (prefix, route) in &loaded {
            if let Some(hp) = &route.upstream_host_port
                && matches!(
                    route.managed_auth.as_deref(),
                    Some(crate::auth::ManagedUpstreamAuth::SpiffeX509(_))
                )
            {
                let peer_id = routes
                    .iter()
                    .find(|r| r.prefix.trim_matches('/') == prefix.as_str())
                    .and_then(|r| {
                        if let Some(SpiffeAuthConfig::X509 {
                            expected_upstream_spiffe_id,
                            ..
                        }) = &r.spiffe
                        {
                ...
                match upstream_peer_id.get(hp) {
                    None => {
                        upstream_peer_id.insert(hp.clone(), (peer_id, prefix.clone()));
                    }
                    Some((existing_id, existing_prefix)) if existing_id != &peer_id => {
                        return Err(ProxyError::Config(format!(
                            "routes '{prefix}' and '{existing_prefix}' share upstream '{hp}' \
                             but have conflicting expected_upstream_spiffe_id values; \
                             use the same peer ID on all X.509 SPIFFE routes to the same upstream"
                        )));
                    }
                    _ => {}
                }
