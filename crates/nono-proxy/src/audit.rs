...
/// Rotation has no associated request, so this writes to the tracing log
/// only — not to the in-memory audit buffer.
pub fn log_svid_rotation(event: &crate::spiffe::SvidRotationEvent) {
    info!(
        target: "nono_proxy::audit",
        route_id = %event.route_id,
        workload_spiffe_id = %event.workload_spiffe_id,
        trust_domain = %event.trust_domain,
        "spiffe x509 svid rotated"
    );
}
