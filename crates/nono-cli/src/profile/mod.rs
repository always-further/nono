...
mod tests {
    ...
    /// Plan 43-06 (Cluster 4 replay of upstream 0748cced): conditional profile
    /// entries with an unknown sibling field (e.g. typo `whenn:`) must be
    /// rejected at parse time rather than silently no-op'd. Replayed verbatim
    /// from upstream's `conditional_profile_entries_reject_unknown_fields`
    /// integration test.
    #[test]
