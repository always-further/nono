...
mod tests {
    ...
    #[test]
    fn test_get_builtin_linux_eti_git_ssh() {
        let profile = get_builtin("linux-eti-git-ssh").expect("Profile not found");
        assert_eq!(profile.meta.name, "linux-eti-git-ssh");
        let command_policies = profile
            .command_policies
            .as_ref()
            .expect("linux ETI profile must define command policies");
        assert!(command_policies.commands.contains_key("git"));
        assert!(command_policies.commands.contains_key("ssh"));
        assert_eq!(command_policies.session_can_use, vec!["git".to_string()]);
    ...
    fn test_list_builtin() {
        ...
        assert!(profiles.contains(&"linux-eti-git-ssh".to_string()));
