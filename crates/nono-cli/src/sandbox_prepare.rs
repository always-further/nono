...
pub(crate) struct PreparedSandbox {
    ...
    /// True iff the NVIDIA-specific procfs grants were installed (i.e. the
    /// host has NVIDIA compute devices AND `--allow-gpu` was permitted).
    /// Drives two downstream policies:
    ///
    /// - `nono run` / `nono shell` auto-enable `capability_elevation` so
    ///   the seccomp-notify supervisor is running.
    /// - The supervisor's `task/<tid>/comm` write fast-path is gated on
    ///   this flag (via `SupervisorConfig::allow_proc_task_comm_write`)
    ///   so explicit-cap-elev sessions that don't have NVIDIA active do
    ///   NOT silently get the comm-write allow.
    ///
    /// On non-Linux platforms this always stays false.
...
/// If the NVIDIA procfs grants applied (`prepared.nvidia_procfs_active`)
/// and `capability_elevation` was not already requested by the user or
/// the profile, turn it on so the seccomp-notify supervisor will install
/// and gate the `/proc/self/task/<tid>/comm` write pattern fast-path.
///
/// Returns `true` iff this call auto-enabled `capability_elevation`.
/// Callers thread that into `ExecutionFlags::auto_capability_elevation`
/// so downstream code selects the non-interactive deny-all approval
/// backend (we don't want surprise `[nono] Grant access? [y/N]` prompts
/// for runtimes that touch paths outside `initial_caps`).
///
/// Only fires when NVIDIA is detected — pure DRM render-node / AMD KFD /
/// WSL2 dxg setups do NOT auto-enable, because they don't need the comm
/// pattern allow and shouldn't pay the per-openat supervisor round-trip.
///
/// Must be called for every supervised entry point that honours
/// `--allow-gpu`: `nono run` and `nono shell`. `nono wrap` is Direct
/// mode and cannot install a supervisor, so callers there must reject
/// NVIDIA `--allow-gpu` instead of trying to auto-enable.
#[cfg(target_os = "linux")]
